...
from web.plugins.redis.plugin import PluginError
from web.plugins.redis.plugin import PluginSettings
from web.plugins.redis.plugin import Plugin
from web.plugins.redis.utils import Annotated
...
class ControlEnviron(ControlBaseModel):
    environ: typing.Dict = pydantic.Field(
        ...,
        title='Environment',
        json_schema_extra=dict(var1='variable1', var2='variable2')
    )
...
class ControlHealthCheck(ControlHealthStatus):
    name: str = pydantic.Field(
        ...
        min_length=1,
        json_schema_extra='Redis'
    )
    details: typing.Dict = pydantic.Field(
        ...,
        title='Health check details',
        json_schema_extra=dict(detail1='detail1', detail2='detail2')
    )
...
class ControlHealthError(ControlBaseModel):
    detail: ControlHealth = pydantic.Field(
        ...,
        title='Health error',
        json_schema_extra=ControlHealth(
            status=False,
            checks=[
                ControlHealthCheck(
                    name='Redis',
...
#         title='status',
#         min_length=1,
#         json_schema_extra='API is up and running'
#     )
